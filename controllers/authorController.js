    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            var author = new Author(
                {
                    first_name: encodeURI(req.body.first_name).replace(/%20/g, ' '),
                    family_name: encodeURI(req.body.family_name).replace(/%20/g, ' '),
...
exports.author_update_get = function(req, res, next) {
    ...
    async.parallel ({
        ...
        if(err) {return next(err); }
        if (results.author==null) { //No Author find
            var err = new Error('Author not found.')
            err.status = 404;
            return next(err);
        }
...
exports.author_update_post = [
    ...
    (req, res, next) => {
        ...
        if(!errors.isEmpty()) {
            ...
            //Get Author and all his Books for form.
            async.parallel({
                author: function(callback) {
                    Author.findById(req.params.id).exec(callback)
                },
